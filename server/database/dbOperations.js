...
const fetchUpcomingFlights = async () => {
    const query = `
        ...
        AND departure_time <= NOW() + INTERVAL '5 days';
    `;
    const { rows } = await pool.query(query);
    return rows;
};
...
const fetchUpcomingHotels = async () => {
    const query = `
        ...
        AND check_in_date <= NOW() + INTERVAL '5 days';
    `;
    const { rows } = await pool.query(query);
    return rows;
};
...
const fetchUpcomingActivities = async () => {
    const query = `
        ...
        AND activity_date <= NOW() + INTERVAL '3 days';
    `;
    const { rows } = await pool.query(query);
    return rows;
};

// Fetch upcoming restaurant reservations (3 days)
const fetchUpcomingRestaurants = async () => {
    const query = `
        SELECT * FROM core.restaurant
        WHERE reservation_date >= NOW()
        AND reservation_date <= NOW() + INTERVAL '3 days';
    `;
    const { rows } = await pool.query(query);
    return rows;
};
...
const fetchUpcomingTransports = async () => {
    const query = `
        SELECT * FROM core.transport
        WHERE pickup_time >= NOW()
        AND pickup_time <= NOW() + INTERVAL '3 days';
    `;
    const { rows } = await pool.query(query);
    return rows;
};
...
// Add a notification
const addNotification = async (owner_id, itinerary_id, message) => {
    const query = `
        INSERT INTO core.notifications (owner_id, itinerary_id, message) 
        VALUES ($1, $2, $3) RETURNING *;
    `;
    const values = [owner_id, itinerary_id, message];
    const { rows } = await pool.query(query, values);
    return rows[0];
};

// Fetch notifications for a user
const fetchNotificationsByUserId = async (user_id) => {
    const query = `
...
module.exports = {
    ...
    fetchUpcomingRestaurants,
    fetchUpcomingTransports,
    addNotification,
